...
int main() {
    ...
    if (filter_type == 5) {
        printf("Enter the output PGM file name for the stretched image: ");
        scanf("%s", output_file);  
    }
    ...
    if (filter_type == 4) {
        ...
        fprintf(ofp, "P5\n%d %d\n%d\n", cols, rows, maxval);  // PGM header
        fwrite(product, sizeof(gray), cols * rows, ofp);       // Image data
        fclose(ofp);
        ...
        free(graymap);
        free(product);
        printf("Histogram stretching applied. Image saved to %s and histogram to histogram_stretched_output.csv\n", output_file);
        return 0;
    }
